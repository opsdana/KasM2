...
function sumArray(arr) {
  ...
  let s = 0
  for (const r of arr) {
    const v = r.nominal ?? r.jumlah ?? r.saldo ?? r.total ?? r.nominalKas ?? r.grandTotal
...
export default function ViewRincianKFPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: err.message || 'Gagal memuat rincian saldo' })
      setData(null)
    } finally {
      setLoading(false)
    }
  ...
  if (data && typeof data === 'object') {
    for (const [k, v] of Object.entries(data)) {
      if (Array.isArray(v) && v.length) sections.push({ title: headerLabel(k), rows: v })
    }
  }
  ...
  return (
    <AppShell>
      <PageHeader
        title="View Rincian Saldo"
        description="Rincian saldo kas fisik"
        actions={
          <button
            onClick={fetchData}
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            ...
            <p className="text-sm">Belum ada data rincian saldo untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-6">
            <StatCard title="Grand Total Saldo Kas" value={formatRupiah(grandTotal)} icon={Wallet} color="purple" />
