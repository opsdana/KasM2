...
export default function CitPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await callApiGet('/cit-atm', { tanggal, kodeWilayah: kw })
      setRows(Array.isArray(res) ? res : res?.rows || res?.data || [])
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal memuat laporan CIT' })
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `cit_${tanggal}${kodeWilayah !== 'ALL' ? `_${kodeWilayah}` : ''}.csv`)
  }
  ...
  return (
    <AppShell>
      <PageHeader
        title="Laporan CIT"
        description="Laporan Cash In Transit"
        actions={
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            ...
            <p className="text-sm">Belum ada data CIT untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
