...
export default function CitAtmRekapPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await callApiGet('/cit-atm', { action: 'rekap', tanggal, kodeWilayah: kw })
      setRows(Array.isArray(res) ? res : res?.rows || res?.data || [])
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal memuat rekap CIT ATM' })
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `cit_atm_rekap_${tanggal}${kodeWilayah !== 'ALL' ? `_${kodeWilayah}` : ''}.csv`)
  }
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            ...
            <p className="text-sm">Belum ada data rekap CIT ATM untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
