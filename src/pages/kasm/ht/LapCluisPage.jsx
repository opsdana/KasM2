...
import { Calendar, RefreshCw, CheckCircle, AlertCircle, MapPin, FileSpreadsheet, Download, Wallet } from 'lucide-react'
...
const CURRENCY_HEADERS = ['saldo', 'nominal', 'jumlah', 'total', 'debet', 'kredit', 'selisih', 'grand', 'vault', 'teller', 'sisa']
...
export default function LapCluisPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await callApiGet('/cluis', { tanggal, kodeWilayah: kw })
      setRows(Array.isArray(res) ? res : res?.rows || res?.data || [])
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal memuat data CLUIS' })
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const summaryTotal = useMemo(() => {
    ...
    for (const r of rows) {
      for (const k of columns) {
        if (isCurrencyHeader(k) && typeof r[k] === 'number') s += r[k]
      }
    }
    return s
  }, [rows, columns])
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `cluis_${tanggal}${kodeWilayah !== 'ALL' ? `_${kodeWilayah}` : ''}.csv`)
  }

  return (
    <AppShell>
      <PageHeader
        title="Sisa Dalam Khasanah (CLUIS)"
        description="Laporan sisa uang dalam Khasanah"
        actions={
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            ...
            <p className="text-sm">Belum ada data CLUIS untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
        <>
          {summaryTotal > 0 && (
            <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <StatCard title="Total Sisa Khasanah" value={formatRupiah(summaryTotal)} icon={Wallet} color="purple" />
            </div>
          )}
