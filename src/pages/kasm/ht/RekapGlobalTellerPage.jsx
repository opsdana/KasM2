...
export default function RekapGlobalTellerPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await callApiGet('/posisi-kas', { action: 'view-posisi', tanggal, kodeWilayah: kw })
      setRows(Array.isArray(res) ? res : res?.rows || res?.data || [])
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal memuat rekap jurnal teller' })
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `rekap_global_teller_${tanggal}${kodeWilayah !== 'ALL' ? `_${kodeWilayah}` : ''}.csv`)
  }

  return (
    <AppShell>
      <PageHeader
        title="Rekap Jurnal Teller"
        description="Rekap global jurnal teller"
        actions={
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            ...
            <p className="text-sm">Belum ada data rekap jurnal teller untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
