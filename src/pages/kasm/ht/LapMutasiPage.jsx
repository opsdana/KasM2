...
import { Calendar, RefreshCw, CheckCircle, AlertCircle, MapPin, FileSpreadsheet, Download, ArrowDownToLine, ArrowUpFromLine } from 'lucide-react'
...
export default function LapMutasiPage() {
  ...
  const currentMode = (mode || 'SETOR').toUpperCase()
  const isSetor = currentMode === 'SETOR'
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await callApiGet('/bon-setor', {
        action: 'history',
        tanggal,
        kodeWilayah: kw,
        tipe: currentMode,
      ...
      setMessage({ type: 'error', text: err.message || 'Gagal memuat mutasi khasanah' })
      setRows([])
    } finally {
      setLoading(false)
    }
  }, [tanggal, kodeWilayah, currentMode])
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `mutasi_${currentMode.toLowerCase()}_${tanggal}${kodeWilayah !== 'ALL' ? `_${kodeWilayah}` : ''}.csv`)
  }

  return (
    <AppShell>
      <PageHeader
        title={isSetor ? 'Setoran Khasanah' : 'Pengeluaran Khasanah'}
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            {isSetor ? <ArrowDownToLine className="h-8 w-8" /> : <ArrowUpFromLine className="h-8 w-8" />}
            <p className="text-sm">Belum ada data mutasi {currentMode} untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
