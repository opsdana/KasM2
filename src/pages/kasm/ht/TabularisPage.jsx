...
import { cn, formatTanggal, exportToCSV, formatRupiah } from '@/lib/utils'
import { useAuth } from '@/contexts/AuthContext'
import AppShell from '@/components/layout/AppShell'
import PageHeader from '@/components/layout/PageHeader'
import LoadingSpinner from '@/components/shared/LoadingSpinner'
import { Calendar, RefreshCw, CheckCircle, AlertCircle, MapPin, Download, FileSpreadsheet } from 'lucide-react'
...
const CURRENCY_HEADERS = ['saldo', 'saldoHariIni', 'saldoFisik', 'selisih', 'jumlah', 'nominal', 'debet', 'kredit']
...
export default function TabularisPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await callApiGet('/tabularis', {
        tanggal,
        kodeWilayah: kw,
      })
      setRows(Array.isArray(res) ? res : [])
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal memuat data tabularis' })
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `tabularis_${tanggal}${kodeWilayah !== 'ALL' ? `_${kodeWilayah}` : ''}.csv`)
  }

  const renderCell = (key, value) => {
    if (value == null) return '-'
    if (typeof value === 'number' && isCurrencyHeader(key)) {
  ...
  return (
    <AppShell>
      <PageHeader
        title="Laporan Tabularis"
        description="Laporan tabularis posisi kas harian"
        actions={
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            ...
            <p className="text-sm">Belum ada data tabularis untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
