...
export default function CisPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await callApiGet('/laporan-cib-cis', { action: 'cis', tanggal, kodeWilayah: kw })
      setRows(Array.isArray(res) ? res : res?.rows || res?.data || [])
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal memuat laporan CIS' })
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = () => {
    ...
    exportToCSV(rows, `cis_${tanggal}${kodeWilayah !== 'ALL' ? `_${kodeWilayah}` : ''}.csv`)
  }
  ...
  return (
    <AppShell>
      <PageHeader
        title="Laporan CIS"
        description="Laporan Cash In Safe"
        actions={
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-gray-400">
            ...
            <p className="text-sm">Belum ada data CIS untuk tanggal ini.</p>
          </div>
        </div>
      ) : (
