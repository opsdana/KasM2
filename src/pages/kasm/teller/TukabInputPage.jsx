...
const BANK_OPTIONS = ['BRI', 'BJB', 'Mandiri', 'BNI', 'BCA', 'CIMB', 'BTN', 'Bukopin', 'Lainnya']
...
export default function TukabInputPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setMessage(null)
    try {
      const data = await callApiGet('/tukab')
      setRows(Array.isArray(data) ? data : [])
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal memuat data TUKAB' })
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const openEditForm = (row) => {
    ...
    setForm({
      ...
      tglTransaksi: tgl,
      bank: row.bank ?? 'BRI',
      nominalTukab: row.nominalTukab ?? '',
      statusAntar: !!row.statusAntar,
    })
    setMessage(null)
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        bank: form.bank,
        nominalTukab: parseNum(form.nominalTukab),
        statusAntar: !!form.statusAntar,
      }
      if (editing) payload.id = editing.id
      await callApiPost('/tukab', payload)
      setMessage({
        type: 'success',
        text: editing ? 'Data TUKAB berhasil diperbarui!' : 'Data TUKAB berhasil ditambahkan!',
      })
      closeForm()
      fetchData()
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal menyimpan data TUKAB' })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleDelete = async (row) => {
    if (!confirm(`Hapus data TUKAB tanggal ${row.tglTransaksi ?? ''} (${row.bank ?? ''})?`)) return
    try {
      await callApiDelete(`/tukab/${row.id}`)
      setMessage({ type: 'success', text: 'Data TUKAB berhasil dihapus!' })
      fetchData()
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Gagal menghapus data TUKAB' })
    }
  }

  const columns = [
    { header: 'Tanggal', accessor: 'tglTransaksi', render: (r) => r.tglTransaksi ? formatTanggal(r.tglTransaksi, 'short') : '-' },
    { header: 'Bank', accessor: 'bank', render: (r) => r.bank || '-' },
    { header: 'Nominal TUKAB', accessor: 'nominalTukab', render: (r) => formatRupiah(r.nominalTukab ?? 0) },
    {
      header: 'Status Antar',
      accessor: 'statusAntar',
      render: (r) => {
        const on = !!r.statusAntar
        return (
  ...
  return (
    <AppShell>
      <PageHeader
        title="Input TUKAB"
        description="Manajemen data penyetoran/antar uang TUKAB"
        actions={
          <button onClick={openAddForm} className="flex items-center gap-2 rounded-lg bg-brand-primary px-4 py-2 text-sm font-medium text-white hover:bg-brand-secondary">
            <Plus className="h-4 w-4" /> Tambah TUKAB
          </button>
        }
      />
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-gray-900">{editing ? 'Edit Data TUKAB' : 'Tambah Data TUKAB'}</h3>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select required value={form.bank} onChange={(e) => setForm((f) => ({ ...f, bank: e.target.value }))} className={inputClass}>
                {BANK_OPTIONS.map((b) => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nominal TUKAB</label>
              <input type="number" min="0" value={form.nominalTukab} onChange={(e) => setForm((f) => ({ ...f, nominalTukab: e.target.value }))} className={inputClass} />
              <p className="text-xs text-gray-400 mt-1">= {formatRupiah(parseNum(form.nominalTukab))}</p>
            </div>
            <div className="flex items-center gap-3 pt-6">
              <label className="inline-flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.statusAntar} onChange={(e) => setForm((f) => ({ ...f, statusAntar: e.target.checked }))} className="h-4 w-4 rounded border-gray-300 text-brand-secondary focus:ring-brand-secondary" />
                <span className="text-sm font-medium text-gray-700">Sudah Antar</span>
              </label>
            </div>
            <div className="sm:col-span-2 flex items-center gap-3 pt-2">
              <button type="submit" disabled={submitting} className="flex items-center gap-2 rounded-lg bg-brand-primary px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-secondary disabled:opacity-60">
                ...
                {submitting ? 'Menyimpan...' : editing ? 'Simpan Perubahan' : 'Tambah TUKAB'}
              </button>
      ...
      <DataTable columns={columns} data={rows} loading={loading} total={rows.length} page={1} limit={rows.length || 25} showPagination={false} emptyMessage="Belum ada data TUKAB" />
    </AppShell>
  )
}